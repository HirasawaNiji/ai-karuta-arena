...
状态：推荐方案，随需求 PR 待评审；本次没有安装应用框架或建立服务。依据：[需求规格](requirements.md)。
...
本 PR 的接受仅确定文档基线与架构方向，不表示玩法效果已验证，也不批准生产部署。首次实现骨架前，评审者可在关联 Issue 选择 A 或提出更改；若改变选择，应同步改路线图与契约归属，不能继续让两套方案同时成为实现指令。
