...
本文 score-v1 是核心 Mock 回归基线。[初赛补充](preliminary-demo.md) PRE-01–03 规定手动标签、分级识别自报、题目/录音范围以及独立的 score-v2-manual；D1 显式选择 v2，不能继续用 v1 的低权重声称“前奏就能认出”已成为高熟悉评分，也不能暗改 v1 测试期望。模型接入不参与这两版计算。

M2 的实际 API、归并细节、年代键映射、范围保留及测试数据见[画像计算交付](m2-profile-matrix.md)。本文选曲/公平公式已在 [M3 交付](m3-fair-selection.md) 实现；运行时/开局流程仍待 M4。
...
84 首是算法测试数据，不能拿它冒充真实音频题库。可玩初赛先过滤经过核验的音源/题目/卡牌；选出正式题组后锁定，随机仅决定其播放顺序。赛事使用赛前画像快照，普通派对下一局使用新反馈，见初赛补充 PRE-07。
