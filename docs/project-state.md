...
- 当前已补齐 M1 工作区、core 领域/画像/选曲/派对/游戏公共契约，以及 M2 Mock 来源、画像和熟悉度矩阵及业务测试；M3 已补独立公平评估与逐首选曲；没有运行时、应用、服务或部署配置。
...
- [架构基线](architecture.md)：轻量 pnpm TypeScript Monorepo、首期单进程 Demo；后续再增加 React/Vite Web 与单个 Node 服务端。M1 契约、M2 music-profile/adapters 和 M3 playlist-engine 已实施；运行时与应用尚未实施。
...
- [验收矩阵](acceptance.md)定义业务测试要求，已进行 M1 契约、M2 画像/评分及 M3 选曲/重评业务检查；状态、Mock 闭环与真人体验仍未验证。
...
- [M1 基础验收](m1-foundation.md)：M1.1–M1.3 与子范围 CI，[M1.4](m1-evidence-contracts.md) 已补证据/画像/矩阵/配置契约，[M1.5/M1.6](m1-runtime-contracts.md) 补齐运行端口与全阶段契约验证，具体接受状态见 PR；[M2](m2-profile-matrix.md) 已补来源、归并、画像和矩阵计算；[M3](m3-fair-selection.md) 已补独立评估和逐首选曲；运行时与 CLI 闭环仍待 M4/M5。
