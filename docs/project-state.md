...
- 当前已补齐 M1 工作区、core 领域/画像/选曲/派对/游戏公共契约，以及 M2 Mock 来源、画像和熟悉度矩阵及业务测试；M3 已补独立公平评估与逐首选曲；M4 已补派对运行时与 Mock 游戏反馈；M5 已补可运行 CLI 和固定场景。D1 已增加独立浏览器入场应用与 Node 服务；D2 已增加真实双人对局；D3 已接入多人同场，D4 已实现 4/8 人淘汰赛；现场真人验收和生产部署尚未完成。
...
2026-09-25 用户要求持续自主开发、测试与工程验收；真人与 QQ 真机未验证项如实单列，不再作为停止工程迭代的前置条件。静音回归见 [浏览器回归](browser-regression.md)。UI 迭代 #33 仅交给用户指定的其他前端特化模型；#30 队战含义保持 Idle，当前 Agent 暂不处理。
...
- [架构基线](architecture.md)：轻量 pnpm TypeScript Monorepo、首期单进程 Demo；后续再增加 React/Vite Web 与单个 Node 服务端。M1 契约、M2 music-profile/adapters、M3 playlist-engine 和 M4 party-runtime/Mock 游戏已实施；M5 CLI 已实施，D1 入场与大厅已实施，D2 双人真实对局已接入。
...
- [验收矩阵](acceptance.md)定义业务测试要求，已进行 M1 契约、M2 画像/评分及 M3 选曲/重评业务检查；M4 状态、模拟游戏与反馈已有集成测试；M5 命令行场景、JSON/文本报告和无密钥进程检查已补齐；D2 已完成真实素材双浏览器自动操作验证，现场真人听辨仍待验收。
...
- [D0 核对](karuta-engine-audit.md)：隔离构建/46 项测试通过，双浏览器完成原引擎 38 回合；106 段音频可解码、105 张卡面可用、1 条缺封面。发现交牌超时不推进并有复现探针。后续 D2 已补包内录音标签/哈希、用户前奏与映射确认、题组冻结和事件适配。用户已授权复用现有内容，官方新卡牌范围仍待确认。
...
D2 实现 10/15 张双人准备、规则适配、服务端裁决、心跳、独立文字牌 UI 与带范围反馈，见 [D2](d2-duel.md)。管理员已确认 PJSK 包片段从录音 0 秒开始且完成曲名/音频人工核对，正式清单启用 105 首；一条同名不同哈希曲目保留待消歧。依据见 [素材确认](material-review-confirmation.md)。实现不代表完整 Demo 已完成，两种局长已完成双浏览器结算；D3/D4 证据见下文，现场体验仍按 Issue 推进。

## D3 多人实现

多人独立规则引擎、全员选曲/BAN/重评、共享音箱、逐题锁定与并列排名已接入，见 [多人说明](d3-multiplayer.md)。三个 Chrome 客户端完成真实 12 题并显示一致排名。后续 D4 已实现，完整现场演示仍待验收。

## D4 赛事实现

已实现 4/8 人固定对阵、真实单场复用、曝光排除、冻结画像、加赛/弃权审计及可信补库，见 [赛事说明](d4-tournament.md)。四个静音 Chrome 完成三场并显示一致冠军；八人七场由真实引擎自动测试覆盖。完整启动与操作见 [演示手册](demo-walkthrough.md)。自动浏览器结果不等于真人听辨或 QQ WebView 真机验收，#14 仍保留该验收边界。
