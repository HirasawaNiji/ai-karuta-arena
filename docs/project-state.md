...
- 当前已补齐 M1 工作区、core 领域/画像/选曲/派对/游戏公共契约，以及 M2 Mock 来源、画像和熟悉度矩阵及业务测试；M3 已补独立公平评估与逐首选曲；M4 已补派对运行时与 Mock 游戏反馈；M5 已补可运行 CLI 和固定场景。D1 已增加独立浏览器入场应用与 Node 服务；真实对局与生产部署尚未完成。
...
- [架构基线](architecture.md)：轻量 pnpm TypeScript Monorepo、首期单进程 Demo；后续再增加 React/Vite Web 与单个 Node 服务端。M1 契约、M2 music-profile/adapters、M3 playlist-engine 和 M4 party-runtime/Mock 游戏已实施；M5 CLI 已实施，D1 入场与大厅已实施，真实对局尚未接入。
...
- [D1 工作单 #21](https://github.com/HirasawaNiji/ai-karuta-arena/issues/21)：已实现 QQ 音乐场景独立 UI、手动画像、可信房间、10/15 张预设及素材试听核验入口，见 [D1 运行说明](d1-lobby.md)。D0 待核验素材不自动变为可玩；D2 继续真实对局适配。
- [周杰伦专场](artist-party.md)：按艺人限定候选，保留公平和禁歌；全曲库完整性与真实可播素材分别核验，不能将 QQ 版权背景视作素材分发授权。

## D2 当前实现

本分支实现 10/15 张双人准备、规则适配、服务端裁决、心跳、独立文字牌 UI 与带范围反馈，见 [D2](d2-duel.md)。管理员已确认 PJSK 包片段从录音 0 秒开始且完成曲名/音频人工核对，正式清单启用 105 首；一条同名不同哈希曲目保留待消歧。依据见 [素材确认](material-review-confirmation.md)。实现不代表完整 Demo 已完成，双浏览器最终验收与 D3/D4 仍按 Issue 推进。
