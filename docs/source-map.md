...
- 后续[详细实现架构](implementation-architecture.md)、[算法规格](algorithm-spec.md)、[运行规格](party-runtime-spec.md)和[工作单](implementation-plan.md)是在已合并需求之上的实施细化，新增默认公式/配置属于工程决策，不冒充原始 Prompt 内容或真实用户验证结果。历史来源快照不修改。
- 2026-09-24 [初赛可玩 Demo 补充](preliminary-demo.md)来自用户后续确认的实际游玩流程与“初赛暂不接大模型”指示，新增 PRE-01–08 / PAD-01–10，不属于原 Prompt 四十九章原文。保留核心 Mock 验收，明确真人画像、题目/卡牌、多人/1v1、单淘汰和后续循环赛的独立范围。
