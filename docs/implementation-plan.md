...
3. 从下表最早未完成且依赖满足的步骤开始。已有阶段分支则恢复，不能另起重复实现。
...
| M1.3 | core/ids、taxonomy、song、player | 开放标签注册/层级校验，歌曲/艺人/八维偏好 schema | AC-01：新增标签无需改枚举、语言地区分离、无 IP、早于 1960 年代 |
...
| M2.5 | build-matrix、gameplay-evidence | 完整 ID 索引矩阵；标准 ANSWER 事件转证据的纯函数 | 同输入稳定；不漏 cell；无动作不生成 game_wrong |
...
| 真实 QQ、Karuta、音频许可、LLM、账户、数据库 | 暂不实现，不阻塞 Mock；后续明确接口/权利/需求后单独规划 |
